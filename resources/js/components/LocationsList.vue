            ...
            <b-table :fields="listTableFields" :items="filteredLocations" class="table table-striped table-hover" :tbody-tr-class="'show-on-hover-container'">
                ...
                <template v-slot:cell(link)="data">
                    <div class="show-on-hover">
                        <a :href="'/locations/' + data.item.id+ '/locations'">
    ...
    export default {
        ...
        computed: {
            locationTypes: function () {
                return _.uniqBy(
                    _.map(
                        this.locations,
                        function (aLocation) {
                            return { label: _.upperFirst(aLocation.location_type_name), code: aLocation.location_type_name}                                                            
                        }
                    ),
                    'code'
                )
            ...
            filteredLocations: function(){
                ...
                if (this.searchTerm) {
                    var filterValue = _.lowerCase(this.searchTerm);
                    locations =  _.filter(
                        locations,
                        function (aLocation) {
        ...
        methods: {
            loadLocations: function(){
                ...
                axiosApi.get('http://geo.apimali.com/v1' + path, {}, {headers: {'origin': window.location.origin}})
